...
FROM app_store_apps
LIMIT 50;
/*APP STORE APPS col: name(text), size_bytes(text), currency(text),
review_count(text), rating (numeric(2, 1))
content_rating(text, primary genre(text)*/

SELECT *
FROM play_store_apps
LIMIT 50;
/* PLAY STORE APPS col: name(text), category(text), rating(number(2, 1)),
review_count(integer), size(text), install_count(text),
type(text), price(text), content_rating(text), genres(text)*/
